...
ACCOUNTS = [
    "uid=5102146855&password=OUT_OF_LAW_1BAD8615FA2084DB410A5EB90331E0AF38A3F09DA22C7B930DD2A",
    "uid=5102146882&password=OUT_OF_LAW_A049B0297F48E411DF87C7403A804E84A983442556C925B09C72C",
    "uid=5102146843&password=OUT_OF_LAW_BE2476DEC6A2F0982CEBB31E159BF414E3DACE3857A61D2D22E6F",
    "uid=5102146957&password=OUT_OF_LAW_6E6F58FB0CCEFB20B7874AF0508BFA828DD98BE0B0C35C6176DDB",
    "uid=5102146982&password=OUT_OF_LAW_F0D11B6E3CBB2BDB7888052F8F43EB03C09236D7420C3E98C042F",
    "uid=5102146966&password=OUT_OF_LAW_3EDD0E402A8A37175E57B2D2879B45CCDAB463DE8D01E61D4338B",
    "uid=5102147129&password=OUT_OF_LAW_FC2F16A31D86D78125951CBA0C03E5BC515F18FCB87C941C7A00F",
    "uid=5102147184&password=OUT_OF_LAW_F362F6B5B31E1BBC57208F701819D0F42DF5D7E0B02FA1B32D2F4",
    "uid=5102147203&password=OUT_OF_LAW_BB3A69241E3C70A84B707E82153DC59C8D513E5C9A0B76D5A0935",
    "uid=5102147215&password=OUT_OF_LAW_46517D6FD07C7247F5F2AA394D2CCCBB24C680743888A0F11FE5E"
]
# === Manager State (Thread-Safe) ===
account_index = 0
account_lock = threading.Lock()
token_pool = {}
token_locks = {i: threading.Lock() for i in range(len(ACCOUNTS))}
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            token_pool[idx] = {
                ...
                'expires_at': time.time() + 25200  # Valid for 7 hours
            }
...
async def get_rotated_token_info() -> Tuple[int, str, str, str]:
    ...
    if idx in token_pool and time.time() < token_pool[idx]['expires_at']:
        info = token_pool[idx]
...
async def GetAccountInformation(uid, unk, endpoint, max_retries=5):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if resp.status_code == 401:
                    with token_locks[acc_idx]:
                        if acc_idx in token_pool:
                            del token_pool[acc_idx]
                    continue 
            ...
            last_error = e
            print(f"Attempt {attempt + 1} failed for account index {acc_idx}. Trying next... Error: {str(e)}")
            continue
            
    raise RuntimeError(f"All fallback attempts failed. Last error: {str(last_error)}")